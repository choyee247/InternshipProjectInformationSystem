// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using ProjectManagementSystem.Data;

#nullable disable

namespace ProjectManagementSystem.Migrations
{
    [DbContext(typeof(ApplicationDbContext))]
    [Migration("20250627040747_AddDatas")]
    partial class AddDatas
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.0")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRole", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("NormalizedName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.HasKey("Id");

                    b.HasIndex("NormalizedName")
                        .IsUnique()
                        .HasDatabaseName("RoleNameIndex")
                        .HasFilter("[NormalizedName] IS NOT NULL");

                    b.ToTable("AspNetRoles", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("ClaimType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("RoleId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetRoleClaims", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("ClaimType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserClaims", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.Property<string>("LoginProvider")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ProviderKey")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ProviderDisplayName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("LoginProvider", "ProviderKey");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserLogins", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("RoleId")
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("UserId", "RoleId");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetUserRoles", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("LoginProvider")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Value")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("UserId", "LoginProvider", "Name");

                    b.ToTable("AspNetUserTokens", (string)null);
                });

            modelBuilder.Entity("ProjectManagementSystem.Models.ApplicationUser", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("nvarchar(450)");

                    b.Property<int>("AccessFailedCount")
                        .HasColumnType("int");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Email")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<bool>("EmailConfirmed")
                        .HasColumnType("bit");

                    b.Property<string>("FullName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<bool>("LockoutEnabled")
                        .HasColumnType("bit");

                    b.Property<DateTimeOffset?>("LockoutEnd")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("NormalizedEmail")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("NormalizedUserName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("PasswordHash")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PhoneNumber")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("PhoneNumberConfirmed")
                        .HasColumnType("bit");

                    b.Property<string>("SecurityStamp")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("TwoFactorEnabled")
                        .HasColumnType("bit");

                    b.Property<string>("UserName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.HasKey("Id");

                    b.HasIndex("NormalizedEmail")
                        .HasDatabaseName("EmailIndex");

                    b.HasIndex("NormalizedUserName")
                        .IsUnique()
                        .HasDatabaseName("UserNameIndex")
                        .HasFilter("[NormalizedUserName] IS NOT NULL");

                    b.ToTable("AspNetUsers", (string)null);
                });

            modelBuilder.Entity("ProjectManagementSystem.Models.AuditLog", b =>
                {
                    b.Property<int>("Log_pkId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Log_pkId"));

                    b.Property<string>("Action")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("PerformedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("PerformedBy")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("StudentName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<int>("Student_pkId")
                        .HasColumnType("int");

                    b.HasKey("Log_pkId");

                    b.HasIndex("Student_pkId");

                    b.ToTable("AuditLogs");
                });

            modelBuilder.Entity("ProjectManagementSystem.Models.Company", b =>
                {
                    b.Property<int>("Company_pkId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Company_pkId"));

                    b.Property<string>("Address")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<string>("CompanyName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("Contact")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<string>("Incharge")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("Company_pkId");

                    b.ToTable("Companies");
                });

            modelBuilder.Entity("ProjectManagementSystem.Models.Framework", b =>
                {
                    b.Property<int>("Framework_pkId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Framework_pkId"));

                    b.Property<string>("FrameworkName")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<int>("Language_pkId")
                        .HasColumnType("int");

                    b.HasKey("Framework_pkId");

                    b.HasIndex("Language_pkId");

                    b.ToTable("Frameworks");

                    b.HasData(
                        new
                        {
                            Framework_pkId = 1,
                            FrameworkName = "ASP.NET Core",
                            Language_pkId = 4
                        },
                        new
                        {
                            Framework_pkId = 2,
                            FrameworkName = "React",
                            Language_pkId = 2
                        },
                        new
                        {
                            Framework_pkId = 3,
                            FrameworkName = "Angular",
                            Language_pkId = 2
                        },
                        new
                        {
                            Framework_pkId = 4,
                            FrameworkName = "Vue.js",
                            Language_pkId = 2
                        },
                        new
                        {
                            Framework_pkId = 5,
                            FrameworkName = "Node.js",
                            Language_pkId = 2
                        },
                        new
                        {
                            Framework_pkId = 6,
                            FrameworkName = "Django",
                            Language_pkId = 1
                        },
                        new
                        {
                            Framework_pkId = 7,
                            FrameworkName = "Flask",
                            Language_pkId = 1
                        },
                        new
                        {
                            Framework_pkId = 8,
                            FrameworkName = "Spring Boot",
                            Language_pkId = 5
                        },
                        new
                        {
                            Framework_pkId = 9,
                            FrameworkName = "Laravel",
                            Language_pkId = 11
                        },
                        new
                        {
                            Framework_pkId = 10,
                            FrameworkName = "Ruby on Rails",
                            Language_pkId = 12
                        },
                        new
                        {
                            Framework_pkId = 11,
                            FrameworkName = "Flutter",
                            Language_pkId = 10
                        },
                        new
                        {
                            Framework_pkId = 12,
                            FrameworkName = "Express.js",
                            Language_pkId = 2
                        },
                        new
                        {
                            Framework_pkId = 13,
                            FrameworkName = "ASP.NET MVC",
                            Language_pkId = 4
                        },
                        new
                        {
                            Framework_pkId = 14,
                            FrameworkName = "Electron",
                            Language_pkId = 2
                        },
                        new
                        {
                            Framework_pkId = 15,
                            FrameworkName = "Svelte",
                            Language_pkId = 2
                        });
                });

            modelBuilder.Entity("ProjectManagementSystem.Models.Language", b =>
                {
                    b.Property<int>("Language_pkId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Language_pkId"));

                    b.Property<string>("LanguageName")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Language_pkId");

                    b.ToTable("Languages");

                    b.HasData(
                        new
                        {
                            Language_pkId = 1,
                            LanguageName = "Python"
                        },
                        new
                        {
                            Language_pkId = 2,
                            LanguageName = "JavaScript"
                        },
                        new
                        {
                            Language_pkId = 3,
                            LanguageName = "TypeScript"
                        },
                        new
                        {
                            Language_pkId = 4,
                            LanguageName = "C#"
                        },
                        new
                        {
                            Language_pkId = 5,
                            LanguageName = "Java"
                        },
                        new
                        {
                            Language_pkId = 6,
                            LanguageName = "Go"
                        },
                        new
                        {
                            Language_pkId = 7,
                            LanguageName = "Rust"
                        },
                        new
                        {
                            Language_pkId = 8,
                            LanguageName = "Kotlin"
                        },
                        new
                        {
                            Language_pkId = 9,
                            LanguageName = "Swift"
                        },
                        new
                        {
                            Language_pkId = 10,
                            LanguageName = "Dart"
                        },
                        new
                        {
                            Language_pkId = 11,
                            LanguageName = "PHP"
                        },
                        new
                        {
                            Language_pkId = 12,
                            LanguageName = "Ruby"
                        },
                        new
                        {
                            Language_pkId = 13,
                            LanguageName = "Scala"
                        },
                        new
                        {
                            Language_pkId = 14,
                            LanguageName = "SQL"
                        },
                        new
                        {
                            Language_pkId = 15,
                            LanguageName = "Shell"
                        });
                });

            modelBuilder.Entity("ProjectManagementSystem.Models.NRCRegionCode", b =>
                {
                    b.Property<int>("NRCRegionCode_pkId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("NRCRegionCode_pkId"));

                    b.Property<string>("RegionCode")
                        .IsRequired()
                        .HasMaxLength(5)
                        .HasColumnType("nvarchar(5)");

                    b.HasKey("NRCRegionCode_pkId");

                    b.ToTable("NRCRegionCodes");

                    b.HasData(
                        new
                        {
                            NRCRegionCode_pkId = 1,
                            RegionCode = "1"
                        },
                        new
                        {
                            NRCRegionCode_pkId = 2,
                            RegionCode = "2"
                        },
                        new
                        {
                            NRCRegionCode_pkId = 3,
                            RegionCode = "3"
                        },
                        new
                        {
                            NRCRegionCode_pkId = 4,
                            RegionCode = "4"
                        },
                        new
                        {
                            NRCRegionCode_pkId = 5,
                            RegionCode = "5"
                        },
                        new
                        {
                            NRCRegionCode_pkId = 6,
                            RegionCode = "6"
                        },
                        new
                        {
                            NRCRegionCode_pkId = 7,
                            RegionCode = "7"
                        },
                        new
                        {
                            NRCRegionCode_pkId = 8,
                            RegionCode = "8"
                        },
                        new
                        {
                            NRCRegionCode_pkId = 9,
                            RegionCode = "9"
                        },
                        new
                        {
                            NRCRegionCode_pkId = 10,
                            RegionCode = "10"
                        },
                        new
                        {
                            NRCRegionCode_pkId = 11,
                            RegionCode = "11"
                        },
                        new
                        {
                            NRCRegionCode_pkId = 12,
                            RegionCode = "12"
                        },
                        new
                        {
                            NRCRegionCode_pkId = 13,
                            RegionCode = "13"
                        },
                        new
                        {
                            NRCRegionCode_pkId = 14,
                            RegionCode = "14"
                        },
                        new
                        {
                            NRCRegionCode_pkId = 15,
                            RegionCode = "15"
                        });
                });

            modelBuilder.Entity("ProjectManagementSystem.Models.NRCTownship", b =>
                {
                    b.Property<int>("NRCTownship_pkId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("NRCTownship_pkId"));

                    b.Property<int>("NRCRegionCodeId")
                        .HasColumnType("int");

                    b.Property<string>("Township")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("NRCTownship_pkId");

                    b.HasIndex("NRCRegionCodeId");

                    b.ToTable("NRCTownships");
                });

            modelBuilder.Entity("ProjectManagementSystem.Models.NRCType", b =>
                {
                    b.Property<int>("NRCType_pkId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("NRCType_pkId"));

                    b.Property<string>("TypeCode")
                        .IsRequired()
                        .HasMaxLength(5)
                        .HasColumnType("nvarchar(5)");

                    b.Property<string>("TypeDescription")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("NRCType_pkId");

                    b.ToTable("NRCTypes");

                    b.HasData(
                        new
                        {
                            NRCType_pkId = 1,
                            TypeCode = "N",
                            TypeDescription = "National"
                        },
                        new
                        {
                            NRCType_pkId = 2,
                            TypeCode = "E",
                            TypeDescription = "Elder"
                        },
                        new
                        {
                            NRCType_pkId = 3,
                            TypeCode = "P",
                            TypeDescription = "Permanent"
                        },
                        new
                        {
                            NRCType_pkId = 4,
                            TypeCode = "C",
                            TypeDescription = "Child"
                        });
                });

            modelBuilder.Entity("ProjectManagementSystem.Models.Project", b =>
                {
                    b.Property<int>("Project_pkId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Project_pkId"));

                    b.Property<int>("Company_pkId")
                        .HasColumnType("int");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(1000)
                        .HasColumnType("nvarchar(1000)");

                    b.Property<int>("Framework_pkId")
                        .HasColumnType("int");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<int>("Language_pkId")
                        .HasColumnType("int");

                    b.Property<string>("ProjectName")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<DateTime>("ProjectSubmittedDate")
                        .HasColumnType("datetime2");

                    b.Property<int>("ProjectType_pkId")
                        .HasColumnType("int");

                    b.Property<string>("Remark")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<string>("SupervisorName")
                        .IsRequired()
                        .HasMaxLength(1000)
                        .HasColumnType("nvarchar(1000)");

                    b.HasKey("Project_pkId");

                    b.HasIndex("Company_pkId");

                    b.HasIndex("Framework_pkId");

                    b.HasIndex("Language_pkId");

                    b.HasIndex("ProjectType_pkId");

                    b.ToTable("Projects");
                });

            modelBuilder.Entity("ProjectManagementSystem.Models.ProjectFile", b =>
                {
                    b.Property<int>("ProjectFile_pkId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ProjectFile_pkId"));

                    b.Property<string>("FilePath")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<long>("FileSize")
                        .HasColumnType("bigint");

                    b.Property<string>("FileType")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<int>("Project_pkId")
                        .HasColumnType("int");

                    b.Property<DateTime>("UploadedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("UploadedBy")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("ProjectFile_pkId");

                    b.HasIndex("Project_pkId");

                    b.ToTable("ProjectFiles");
                });

            modelBuilder.Entity("ProjectManagementSystem.Models.ProjectMember", b =>
                {
                    b.Property<int>("Project_pkId")
                        .HasColumnType("int");

                    b.Property<int>("Student_pkId")
                        .HasColumnType("int");

                    b.Property<int>("ProjectMember_pkId")
                        .HasColumnType("int");

                    b.HasKey("Project_pkId", "Student_pkId");

                    b.HasIndex("Student_pkId");

                    b.ToTable("ProjectMembers");
                });

            modelBuilder.Entity("ProjectManagementSystem.Models.ProjectType", b =>
                {
                    b.Property<int>("ProjectType_pkId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ProjectType_pkId"));

                    b.Property<string>("TypeName")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("ProjectType_pkId");

                    b.ToTable("ProjectTypes");

                    b.HasData(
                        new
                        {
                            ProjectType_pkId = 1,
                            TypeName = "Web Application"
                        },
                        new
                        {
                            ProjectType_pkId = 2,
                            TypeName = "Mobile Application"
                        },
                        new
                        {
                            ProjectType_pkId = 3,
                            TypeName = "Desktop Application"
                        },
                        new
                        {
                            ProjectType_pkId = 4,
                            TypeName = "API Development"
                        },
                        new
                        {
                            ProjectType_pkId = 5,
                            TypeName = "Library / SDK"
                        },
                        new
                        {
                            ProjectType_pkId = 6,
                            TypeName = "Microservices"
                        },
                        new
                        {
                            ProjectType_pkId = 7,
                            TypeName = "Cloud Service"
                        },
                        new
                        {
                            ProjectType_pkId = 8,
                            TypeName = "Embedded System"
                        },
                        new
                        {
                            ProjectType_pkId = 9,
                            TypeName = "IoT Application"
                        },
                        new
                        {
                            ProjectType_pkId = 10,
                            TypeName = "Game Development"
                        },
                        new
                        {
                            ProjectType_pkId = 11,
                            TypeName = "Progressive Web App (PWA)"
                        },
                        new
                        {
                            ProjectType_pkId = 12,
                            TypeName = "Cross-platform Application"
                        },
                        new
                        {
                            ProjectType_pkId = 13,
                            TypeName = "Single Page Application (SPA)"
                        },
                        new
                        {
                            ProjectType_pkId = 14,
                            TypeName = "Enterprise Application"
                        },
                        new
                        {
                            ProjectType_pkId = 15,
                            TypeName = "Chatbot / AI Application"
                        });
                });

            modelBuilder.Entity("ProjectManagementSystem.Models.Student", b =>
                {
                    b.Property<int>("Student_pkId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Student_pkId"));

                    b.Property<string>("AcademicYear")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreatedDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<string>("Major")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("NRCNumber")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("NRCRegionCode_pkId")
                        .HasColumnType("int");

                    b.Property<int>("NRCTownship_pkId")
                        .HasColumnType("int");

                    b.Property<int>("NRCType_pkId")
                        .HasColumnType("int");

                    b.Property<string>("PhoneNumber")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("RollNumber")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("StudentDepartmentDepartment_pkID")
                        .HasColumnType("int");

                    b.Property<string>("StudentName")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Student_pkId");

                    b.HasIndex("NRCRegionCode_pkId");

                    b.HasIndex("NRCTownship_pkId");

                    b.HasIndex("NRCType_pkId");

                    b.HasIndex("StudentDepartmentDepartment_pkID");

                    b.ToTable("Students");
                });

            modelBuilder.Entity("ProjectManagementSystem.Models.StudentDepartment", b =>
                {
                    b.Property<int>("Department_pkID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Department_pkID"));

                    b.Property<string>("DepartmentName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("Department_pkID");

                    b.ToTable("StudentDepartments");

                    b.HasData(
                        new
                        {
                            Department_pkID = 1,
                            DepartmentName = "Computer Science"
                        },
                        new
                        {
                            Department_pkID = 2,
                            DepartmentName = "Information Technology"
                        });
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.HasOne("ProjectManagementSystem.Models.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.HasOne("ProjectManagementSystem.Models.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("ProjectManagementSystem.Models.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.HasOne("ProjectManagementSystem.Models.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("ProjectManagementSystem.Models.AuditLog", b =>
                {
                    b.HasOne("ProjectManagementSystem.Models.Student", "Student")
                        .WithMany()
                        .HasForeignKey("Student_pkId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Student");
                });

            modelBuilder.Entity("ProjectManagementSystem.Models.Framework", b =>
                {
                    b.HasOne("ProjectManagementSystem.Models.Language", "Language")
                        .WithMany("Frameworks")
                        .HasForeignKey("Language_pkId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Language");
                });

            modelBuilder.Entity("ProjectManagementSystem.Models.NRCTownship", b =>
                {
                    b.HasOne("ProjectManagementSystem.Models.NRCRegionCode", "RegionCode")
                        .WithMany("Townships")
                        .HasForeignKey("NRCRegionCodeId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("RegionCode");
                });

            modelBuilder.Entity("ProjectManagementSystem.Models.Project", b =>
                {
                    b.HasOne("ProjectManagementSystem.Models.Company", "Company")
                        .WithMany()
                        .HasForeignKey("Company_pkId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("ProjectManagementSystem.Models.Framework", "Framework")
                        .WithMany("Projects")
                        .HasForeignKey("Framework_pkId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("ProjectManagementSystem.Models.Language", "Language")
                        .WithMany("Projects")
                        .HasForeignKey("Language_pkId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("ProjectManagementSystem.Models.ProjectType", "ProjectType")
                        .WithMany()
                        .HasForeignKey("ProjectType_pkId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Company");

                    b.Navigation("Framework");

                    b.Navigation("Language");

                    b.Navigation("ProjectType");
                });

            modelBuilder.Entity("ProjectManagementSystem.Models.ProjectFile", b =>
                {
                    b.HasOne("ProjectManagementSystem.Models.Project", "Project")
                        .WithMany("Files")
                        .HasForeignKey("Project_pkId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Project");
                });

            modelBuilder.Entity("ProjectManagementSystem.Models.ProjectMember", b =>
                {
                    b.HasOne("ProjectManagementSystem.Models.Project", "Project")
                        .WithMany("Members")
                        .HasForeignKey("Project_pkId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("ProjectManagementSystem.Models.Student", "Student")
                        .WithMany()
                        .HasForeignKey("Student_pkId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Project");

                    b.Navigation("Student");
                });

            modelBuilder.Entity("ProjectManagementSystem.Models.Student", b =>
                {
                    b.HasOne("ProjectManagementSystem.Models.NRCRegionCode", "NRCRegionCode")
                        .WithMany()
                        .HasForeignKey("NRCRegionCode_pkId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("ProjectManagementSystem.Models.NRCTownship", "NRCTownship")
                        .WithMany()
                        .HasForeignKey("NRCTownship_pkId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("ProjectManagementSystem.Models.NRCType", "NRCType")
                        .WithMany()
                        .HasForeignKey("NRCType_pkId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("ProjectManagementSystem.Models.StudentDepartment", null)
                        .WithMany("Students")
                        .HasForeignKey("StudentDepartmentDepartment_pkID");

                    b.Navigation("NRCRegionCode");

                    b.Navigation("NRCTownship");

                    b.Navigation("NRCType");
                });

            modelBuilder.Entity("ProjectManagementSystem.Models.Framework", b =>
                {
                    b.Navigation("Projects");
                });

            modelBuilder.Entity("ProjectManagementSystem.Models.Language", b =>
                {
                    b.Navigation("Frameworks");

                    b.Navigation("Projects");
                });

            modelBuilder.Entity("ProjectManagementSystem.Models.NRCRegionCode", b =>
                {
                    b.Navigation("Townships");
                });

            modelBuilder.Entity("ProjectManagementSystem.Models.Project", b =>
                {
                    b.Navigation("Files");

                    b.Navigation("Members");
                });

            modelBuilder.Entity("ProjectManagementSystem.Models.StudentDepartment", b =>
                {
                    b.Navigation("Students");
                });
#pragma warning restore 612, 618
        }
    }
}
